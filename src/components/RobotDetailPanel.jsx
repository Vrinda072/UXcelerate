...
export default function RobotDetailPanel({ robot, now, mode, started }) {
  ...
  return (
    <div className="p-4 border-b border-[var(--line)]">
      ...
      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold tracking-wide mb-3 ${TONE_PILL_CLASS[tone]}`}>
        {label.toUpperCase()}
      </span>
      ...
      {!started ? (
        <p className="mt-3 text-[10.5px] leading-snug text-[var(--text-lo)] bg-[var(--ink-800)] border border-[var(--line)] rounded-md px-2.5 py-1.5">
        ...
        <p className="mt-3 text-[10.5px] leading-snug text-[var(--accent)] bg-[var(--accent-dim)] border border-[var(--accent)]/25 rounded-md px-2.5 py-1.5">
          Click anywhere checked on the map to redirect {robot.name}.
        </p>
      ) : (
        <p className="mt-3 text-[10.5px] leading-snug text-[var(--danger)] bg-[var(--danger)]/10 border border-[var(--danger)]/25 rounded-md px-2.5 py-1.5">
